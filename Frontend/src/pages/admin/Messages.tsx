          ...
          {loading ? (
            ...
            chats.map(chat => (
              <button
                ...
                className={`w-full p-4 flex items-center space-x-3 hover:bg-gray-50 transition-colors ${selectedChat?.id === chat.id ? 'bg-indigo-50 border-r-4 border-indigo-600' : ''}`}
              >
                <div className="h-12 w-12 rounded-full bg-indigo-100 flex items-center justify-center text-indigo-600 font-bold">
                  <User size={20} />
                </div>
                <div className="text-left overflow-hidden flex-1">
                  <h3 className="font-bold text-gray-900 truncate">{chat.customer_name}</h3>
          ...
          <>
            ...
            <div className="p-4 border-b border-gray-100 flex items-center justify-between">
              <div className="flex items-center space-x-3">
                ...
                <div>
                  <h3 className="font-bold text-gray-900">{selectedChat.customer_name}</h3>
            ...
            <div className="flex-1 overflow-y-auto p-6 space-y-4 bg-gray-50/50">
              {messages.map((msg, i) => (
                ...
                >
                  <div className={`max-w-[70%] p-3 rounded-2xl shadow-sm ${msg.sender_type === 'business' ? 'bg-indigo-600 text-white rounded-tr-none' : 'bg-white text-gray-900 rounded-tl-none border border-gray-100'}`}>
                    <p className="text-sm leading-relaxed">{msg.text}</p>
                    <p className={`text-[10px] mt-1 flex justify-end ${msg.sender_type === 'business' ? 'text-indigo-100' : 'text-gray-400'}`}>
